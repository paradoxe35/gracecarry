...
export default async function updateProfile(
  ...
  if (data.currentPassword && data.newPassword) {
    const updatePasswordSchema = z
      .object({
        ...
        newPassword: zStrongPasswordSchema(
      ...
      .refine((data) => data.currentPassword && !data.newPassword, {
        message:
          "You must provide a new password if you want to change your current password",
        path: ["newPassword"],
      });
  ...
  try {
    ...
    if (!customer) return { error: "you need be logged in first", data };
    ...
    await revalidateTag(await getCacheTag("customers"));
    // if (data.email !== email) {
