...
const MedusaProductCard = ({ product }: {product: StoreProduct}) => {
  ...
  return (
    ...
    >
      <LocalizedLink href={`/productx/${product.id}`}>
        ...
        <div className="p-4">
          <div className="text-sm text-neutral-600 mb-1">{product.categories?.join(" / ")??""}</div>
          <h3 className="font-medium text-lg mb-2 group-hover:text-primary transition-colors">{product.title}</h3>
          <div className="flex items-center">
            {hasDiscount ? (
              <>
                <span className="font-medium text-primary">${discountedPrice.toFixed(2)}</span>
                <span className="text-neutral-500 line-through ml-2 text-sm">{productPrice?.priceDisplay}</span>
              </>
            ) : (
              <span className="font-medium text-neutral-900">{productPrice?.priceDisplay}</span>
            )}
          </div>
        </div>
      </LocalizedLink>
